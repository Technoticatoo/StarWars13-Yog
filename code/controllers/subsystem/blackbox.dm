...
/datum/controller/subsystem/blackbox/proc/LogBroadcast(freq)
	...
	switch(freq)
		...
		if(FREQ_REBCOM)
			record_feedback("tally", "radio_usage", 1, "RebCom")
		if(FREQ_REBMIL)
			record_feedback("tally", "radio_usage", 1, "RebMil")
		if(FREQ_REBMED)
			record_feedback("tally", "radio_usage", 1, "RebMed")
		if(FREQ_REGSEC)
			record_feedback("tally", "radio_usage", 1, "RebSec")
		if(FREQ_REBSQD)
