...
/obj/item/stack/medical/attack(mob/living/M, mob/user)
	...
	if(iscarbon(M))
		...
		var/choice = show_radial_menu(user, C, radial_options, null, 48, null, TRUE)
		switch(choice)
			if("head")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_HEAD))
			if("chest")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_CHEST))
			if("groin")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_PRECISE_GROIN))
			if("Larm")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_L_ARM))
			if("Lhand")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_PRECISE_L_HAND))
			if("Rarm")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_R_ARM))
			if("Rhand")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_PRECISE_R_HAND))
			if("Lleg")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_L_LEG))
			if("Lfoot")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_PRECISE_L_FOOT))
			if("Rleg")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_R_LEG))
			if("Rfoot")
				affecting = C.get_bodypart(check_zone(BODY_ZONE_PRECISE_R_FOOT))
