...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			if("droid_markings" in pref_species.default_features)
				if(!mutant_category)
					dat += APPEARANCE_CATEGORY_COLUMN

				dat += "<h3>Droid Markings</h3>"

				dat += "<a href='?_src_=prefs;preference=droid_markings;task=input'>[features["droid_markings"]]</a><BR>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("droid_markings")
					var/new_droid_markings
					new_droid_markings = input(user, "Choose your droid's markings:", "Character Preference") as null|anything in GLOB.droid_markings_list
					if(new_droid_markings)
						features["droid_markings"] = new_droid_markings
