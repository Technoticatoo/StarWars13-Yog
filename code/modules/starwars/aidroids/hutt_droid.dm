...
/mob/living/simple_animal/bot/hutt_droid
	name = "\improper Hutt BT16 droid"
	desc = "A bt16 robot. Creepy!."
	icon = 'icons/starwars/mob/droids.dmi'
	icon_state = "bt-16_mob"
	density = FALSE
	anchored = FALSE
	health = 50
	maxHealth = 50
...
/mob/living/simple_animal/bot/hutt_droid/mysterious
	name = "\improper Mysterious BT16 Hutt droid"
	desc = "Hutt BT16 droid of mystery."
	skin = "bezerk"
	treatment_brute = "tricordrazine"
	treatment_fire = "tricordrazine"
	treatment_tox = "tricordrazine"

/mob/living/simple_animal/bot/hutt_droid/derelict
	name = "\improper Old BT16 droid"
	desc = "Looks like it has been here since a long long time ago. It's not very far away though."
	skin = "bezerk"
	heal_threshold = 0
	declare_crit = 0
...
/mob/living/simple_animal/bot/hutt_droid/update_icon()
	cut_overlays()
	if(skin)
...
/mob/living/simple_animal/bot/hutt_droid/Initialize(mapload, new_skin)
	. = ..()
...
/mob/living/simple_animal/bot/hutt_droid/update_mobility()
	. = ..()
	update_icon()

/mob/living/simple_animal/bot/hutt_droid/bot_reset()
	..()
	patient = null
	oldpatient = null
	oldloc = null
...
/mob/living/simple_animal/bot/hutt_droid/proc/soft_reset() //Allows the medibot to still actively perform its medical duties without being completely halted as a hard reset does.
	path = list()
	patient = null
	mode = BOT_IDLE
	last_found = world.time
...
/mob/living/simple_animal/bot/hutt_droid/set_custom_texts()
...
/mob/living/simple_animal/bot/hutt_droid/attack_paw(mob/user)
	return attack_hand(user)

/mob/living/simple_animal/bot/hutt_droid/get_controls(mob/user)
	var/dat
	dat += hack(user)
	dat += showpai(user)
...
/mob/living/simple_animal/bot/hutt_droid/Topic(href, href_list)
	if(..())
		return 1
...
/mob/living/simple_animal/bot/hutt_droid/attackby(obj/item/W as obj, mob/user as mob, params)
...
/mob/living/simple_animal/bot/hutt_droid/emag_act(mob/user)
	..()
	if(emagged == 2)
		declare_crit = 0
		if(user)
...
/mob/living/simple_animal/bot/hutt_droid/process_scan(mob/living/carbon/human/H)
	if(H.stat == DEAD)
		return
...
/mob/living/simple_animal/bot/hutt_droid/handle_automated_action()
	if(!..())
		return
...
/mob/living/simple_animal/bot/hutt_droid/proc/assess_patient(mob/living/carbon/C)
...
/mob/living/simple_animal/bot/hutt_droid/UnarmedAttack(atom/A)
	if(iscarbon(A))
...
/mob/living/simple_animal/bot/hutt_droid/examinate(atom/A as mob|obj|turf in view())
	..()
	if(!is_blind(src))
		chemscan(src, A)

/mob/living/simple_animal/bot/hutt_droid/proc/medicate_patient(mob/living/carbon/C)
	if(!on)
		return
...
/mob/living/simple_animal/bot/hutt_droid/proc/check_overdose(mob/living/carbon/patient,reagent_id,injection_amount)
...
/mob/living/simple_animal/bot/hutt_droid/explode()
	on = FALSE
...
/mob/living/simple_animal/bot/hutt_droid/proc/declare(crit_patient)
...
/obj/machinery/bot_core/hutt_droid
