/obj/effect/proc_holder/spell/targeted/forceln
	name = "Force Lightning"
	desc = "Charge up a lightning blast and release it at a random nearby target! You can move freely while it charges. The arc jumps between targets and can knock them down."
	charge_type = "recharge"
	charge_max	= 300
	clothes_req = FALSE
	invocation = "Force Lightning!"
...
/obj/effect/proc_holder/spell/targeted/forceln/proc/StartChargeup(mob/user = usr)
	ready = TRUE
...
/obj/effect/proc_holder/spell/targeted/forceln/proc/Reset(mob/user = usr)
	ready = FALSE
	user.cut_overlay(halo)

/obj/effect/proc_holder/spell/targeted/forceln/revert_cast(mob/user = usr, message = 1)
	if(message)
		to_chat(user, "<span class='notice'>No target found in range.</span>")
	Reset(user)
	..()

/obj/effect/proc_holder/spell/targeted/forceln/cast(list/targets, mob/user = usr)
	ready = FALSE
...
/obj/effect/proc_holder/spell/targeted/forceln/proc/Bolt(mob/origin,mob/target,bolt_energy,bounces,mob/user = usr)
	origin.Beam(target,icon_state="lightning[rand(1,12)]",time=5)
	var/mob/living/carbon/current = target
	if(current.anti_magic_check())
		playsound(get_turf(current), 'sound/magic/lightningshock.ogg', 50, 1, -1)
		current.visible_message("<span class='warning'>[current] absorbs the lightning, remaining unharmed!</span>", "<span class='userdanger'>You absorb the lightning, remaining unharmed!</span>")
	if(bounces < 1)
