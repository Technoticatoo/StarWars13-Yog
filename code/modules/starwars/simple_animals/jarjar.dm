...
/mob/living/simple_animal/parrot
	...
	//Headset for jarjar to yell at engineers :)
...
/mob/living/simple_animal/parrot/attackby(obj/item/O, mob/living/user, params)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O, /obj/item/reagent_containers/food/snacks/cracker))
		if(O.force)
			...
			icon_state = icon_living
			drop_held_item(0)
	else if(istype(O, /obj/item/reagent_containers/food/snacks/cracker)) //jarjar wants a cracker.
		qdel(O)
		if(health < maxHealth)
			adjustBruteLoss(-10)
 ...
 * Sub-types
 */
/mob/living/simple_animal/parrot/jarjar
	name = "Jar Jar"
	desc = "Jar Jar Binks. An expert on quantum cracker theory."
	speak = list("Jar Jar wanna cracker!", ":e Check the crystal, you chucklefucks!",":e Wire the solars, you lazy bums!",":e WHO TOOK THE DAMN HARDSUITS?",":e OH GOD ITS ABOUT TO DELAMINATE CALL THE SHUTTLE")
	icon = 'icons/starwars/mob/jarjar.dmi'
	icon_state = "jarjar"
	icon_living = "jarjar"
	icon_dead = "jarjar_dead"
	icon_sit = "jarjart_sit"
...
/mob/living/simple_animal/parrot/jarjar/Initialize()
...
/mob/living/simple_animal/parrot/jarjar/Life()
	if(!stat && SSticker.current_state == GAME_STATE_FINISHED && !memory_saved)
		Write_Memory(FALSE)
		memory_saved = TRUE
	..()

/mob/living/simple_animal/parrot/jarjar/death(gibbed)
	if(!memory_saved)
		Write_Memory(TRUE)
	if(rounds_survived == longest_survival || rounds_survived == longest_deathstreak || prob(0.666))
		var/mob/living/simple_animal/parrot/jarjar/ghost/G = new(loc)
		if(mind)
			mind.transfer_to(G)
		else
			G.key = key
...
/mob/living/simple_animal/parrot/jarjar/proc/Read_Memory()
	if(fexists("data/npc_saves/jarjar.sav")) //legacy compatability to convert old format to new
		var/savefile/S = new /savefile("data/npc_saves/jarjar.sav")
		S["phrases"] 			>> speech_buffer
		S["roundssurvived"]		>> rounds_survived
		S["longestsurvival"]	>> longest_survival
		S["longestdeathstreak"] >> longest_deathstreak
		fdel("data/npc_saves/jarjar.sav")
	else
		var/json_file = file("data/npc_saves/jarjar.json")
...
/mob/living/simple_animal/parrot/jarjar/proc/Write_Memory(dead)
	var/json_file = file("data/npc_saves/jarjar.json")
...
/mob/living/simple_animal/parrot/jarjar/ratvar_act()
...
/mob/living/simple_animal/parrot/jarjar/ghost
	name = "The Force Ghost of Jar Jar"
	desc = "Doomed to roam forever."
	color = "#FFFFFF77"
	speak_chance = 20
...
/mob/living/simple_animal/parrot/jarjar/ghost/Initialize()
	memory_saved = TRUE //At this point nothing is saved
	. = ..()

/mob/living/simple_animal/parrot/jarjar/ghost/handle_automated_speech()
	if(ismob(loc))
		return
	..()

/mob/living/simple_animal/parrot/jarjar/ghost/handle_automated_movement()
	if(isliving(parrot_interest))
...
/mob/living/simple_animal/parrot/jarjar/ghost/proc/Possess(mob/living/carbon/human/H)
	if(!ishuman(H))
		return
