...
/obj/spacepod/update_icon()
	...
	if(construction_state != SPACEPOD_ARMOR_WELDED)
		...
		if(pod_armor && construction_state >= SPACEPOD_ARMOR_LOOSE)
			var/mutable_appearance/masked_armor = mutable_appearance(icon = 'icons/starwars/spacepods/construction_2x2.dmi', icon_state = "armor_mask")
	...
	for(var/cdir in GLOB.cardinals)
		...
		if(left_thrust)
			add_overlay(image(icon = 'icons/starwars/YOG/2x2.dmi', icon_state = "rcs_left", dir = cdir))
		if(right_thrust)
			add_overlay(image(icon = 'icons/starwars/YOG/2x2.dmi', icon_state = "rcs_right", dir = cdir))
	if(back_thrust)
		var/image/I = image(icon = 'icons/starwars/YOG/2x2.dmi', icon_state = "thrust")
...
/* /obj/spacepod/verb/select_tracking()
	...
	if(podtype == "empire")
		for(var/obj/structure/starwars/buoy/empire/target in range(range,src))
			...
			targets += target
	if(podtype == "rebels")
		for(var/obj/structure/starwars/buoy/rebels/target in range(range,src))
			targets += target
	if(podtype == "mercs")
		for(var/obj/structure/starwars/buoy/mercs/target in range(range,src))
			targets += target
...
/obj/spacepod/verb/engage_hyperspace()
	...
	if(istype(loc, /turf/open/space) && isinsector)
		...
		addtimer(CALLBACK(src, .proc/hyperjump, src), 300)
	else
...
/obj/spacepod/verb/warp_planets()
	...
	if(podtype == "empire")
		if(istype(loc, /turf/open/space/arrival_sector))
			to_chat(usr, "The ship is already at the desired destination")
		else if(!istype(loc, /turf/open/floor/starwars/hyperspace) && !isinsector)
